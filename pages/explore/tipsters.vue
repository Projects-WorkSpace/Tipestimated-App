...
} = await useAsyncQuery<ITipsters>(TipsterFollowers, { first: 10 });

const searchTipsters = async () => {
  loading_search.value = true;
  const { data, error } = await useAsyncQuery<ITipsters>(TipsterFollowers, {
    penName: searchText.value,
  });
  if (data.value) {
    loading_search.value = false;
    searchedData.value =
      data.value.allTipsters.edges?.map((edge) => edge.node) ?? [];
  }
  if (error.value) {
    loading_search.value = false;
  }
...
const fetchSuggestionsData = computed(() => {
  return (
    fetchSuggestions.value.allTipsters.edges?.map((edge) => edge.node) ?? []
  );
});
...
<template>
  <div class="w-full flex flex-col py-4 md:py-6 gap-y-6 mt-14 md:mt-0">
    ...
    <div v-if="searchText" class="w-full flex flex-col gap-y-2 mt-5">
      <div v-if="searchedData.length > 0" class="w-full">
