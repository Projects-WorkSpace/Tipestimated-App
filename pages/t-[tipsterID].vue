...
import { GetTipster } from "~/graphql/accounts";
import { ITipsterData, ISingleTipster } from "~/types/accounts";
...
const getTipster = async () => {
  ...
  const { onResult, onError } = useQuery<ITipsterData>(GetTipster, {
    tipsterId: route.params.tipsterID,
  });
  onResult((result) => {
    loadingTipster.value = false;
    tipster.value = result.data.getTipster;
  });
  onError((error) => {
    loadingTipster.value = false;
...
const updateFollowStatus = (payload: boolean) => {
  if (tipster.value) {
    const newTipster = { ...tipster.value };
    newTipster.isFollowedByUser = payload;
    payload ? (newTipster.followerCount += 1) : (newTipster.followerCount -= 1);

    tipster.value = newTipster;
  }
};
...
<template>
  <div class="w-full">
    <NuxtLayout name="users">
      <div
        class="w-full flex flex-col items-center gap-y-2 md:gap-y-4 py-3 md:py-8 mt-6 md:mt-0"
      >
        <Transition>
          <div v-if="!loadingTipster" class="w-full">
            <div v-if="tipster" class="w-full">
              <ContainersTipsterHeader
                :tipster="tipster"
          ...
          <div v-else class="flex items-center space-x-4 w-full md:w-[86%]">
            ...
            <div class="space-y-4 grow">
              <USkeleton
                class="h-7 w-full"
                :ui="{ background: 'bg-c-seperator/40' }"
              />
              <USkeleton
                class="h-7 w-full"
                :ui="{ background: 'bg-c-seperator/40' }"
        ...
        <div class="w-full mt-4 md:mt-8">
          <div
            v-if="tipster"
            class="w-full flex items-center gap-x-2 md:gap-x-6"
          >
            <button class="flex items-center px-2 gap-x-1">
              <Icon :name="getIcon(tipster.favoriteSport)" size="15" />
              <p class="text-base ml-1">{{ tipster.favoriteSport }}</p>
            </button>
            <button
              v-for="(sport, index) in otherSportList"
              class="flex items-center px-2 gap-x-1"
              :key="index"
            >
              <Icon :name="getIcon(sport)" size="15" />
              <p class="text-base ml-1">{{ sport }}</p>
            </button>
          </div>
        </div>
