  ...
  if (data.data) {
    if (data.data?.likePost.liked) {
      ...
      setTimeout(() => {
        ...
        emits("updateLike", data.data?.likePost.liked as boolean, props.statusType, props.node?.id as string)
      }, 400);
    } else {
      animations.value.startUnfillAnimation = false;
      emits("updateLike", data.data?.likePost.liked, props.statusType, props.node?.id as string)
    }
  }
})
      ...
      <div class="w-full flex items-center justify-between sm:px-4 pt-1">
        ...
        <div class="flex items-center">
          <div class="flex items-center gap-x-2.5 sm:gap-x-4 rounded-xl px-3 py-3 divide-x divide-c-seperator">
            <div v-if="node?.predictionpostitemSet.edges" class="flex items-center">
              <NuxtImg :src="node?.predictionpostitemSet.edges[0].node.bookmakerImg" class="h-6 sm:h-7 rounded" />
            </div>
