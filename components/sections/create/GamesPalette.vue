    ...
    <div class="w-full h-[60vh] overflow-y-auto flex flex-col ">
      <Transition mode="out-in">
        ...
        <div v-else class="w-full flex flex-col">
          <div v-if="!filteredEvents" class="w-full flex flex-col">
            No data
          </div>
              ...
              <div @click="selectEvent(event)"
                ...
                <div class="flex items-center gap-x-4">
                  <div class="flex items-center gap-x-1">
                    <small class="text-xs text-neutral-500">{{ formatTimestamp(event.START_TIME) }}</small>
                  </div>
                  <div class="flex flex-col gap-y-1">
                    <div class="flex items-center gap-x-2.5">
                      <NuxtImg :src="event.HOME_IMAGES[0] || ''" class="w-5 h-5" />
                      <span class="text-sm text-neutral-600">{{ event.HOME_NAME }}</span>
                    </div>
                    <div class="flex items-center gap-x-2.5">
                      <NuxtImg :src="event.AWAY_IMAGES[0] || ''" class="w-5 h-5" />
                      <span class="text-sm text-neutral-600">{{ event.AWAY_NAME }}</span>
                    </div>

                  </div>
                </div>
                <div class="flex items-center">
                  <UIcon name="i-heroicons-chevron-right-20-solid" class="w-5 h-5 ms-auto text-neutral-500" />
                </div>
              </div>
            </li>
          </ul>
  ...
  if (props.events) {
    if (query.value !== '') {
      ...
      return props.events.EVENTS.filter((event) => {
        const homeNameMatch = event.HOME_NAME.toLowerCase().includes(lowercaseQuery);
        const awayNameMatch = event.AWAY_NAME.toLowerCase().includes(lowercaseQuery);
