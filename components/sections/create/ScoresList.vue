    ...
    <div class="w-full px-4 flex justify-between items-center pt-1 pb-2.5">
      <div class="flex items-center gap-x-3">
        ...
        <div class="flex gap-x-1 items-center">
          <NuxtImg :src="matchData?.HOME_IMAGES[0] || ''" class="w-6 sm:w-5 h-6 sm:h-5" />
          <span class="hidden sm:block text-base font-medium">{{
            matchData?.HOME_NAME
          }}</span>
        </div>
        <span class="text-sm font-normal tracking-wide">-</span>
        <div class="flex gap-x-1 items-center">
          <NuxtImg :src="matchData?.AWAY_IMAGES[0] || ''" class="w-6 sm:w-5 h-6 sm:h-5" />
          <span class="hidden sm:block text-base font-medium">{{
            matchData?.AWAY_NAME
          }}</span>
        </div>
      ...
      <div class="flex items-center gap-x-2">
        <button @click="closeModal" class="px-2 py-1 text-red-400 hover:text-red-500 transition-colors duration-200">
          Cancel
        </button>
        <button @click="closeModal"
          class="px-4 py-1.5 rounded-md text-sm text-white bg-neutral-600 tracking-wide hover:bg-neutral-700 transition-colors duration-200">
          Save
        </button>
      </div>
    </div>
