<template>
  <div class="w-full bg-white flex flex-col gap-y-2 py-2">
    ...
    <ul class="w-full flex flex-col max-h-[34vh] overflow-y-auto">
      <li v-for="(country, index) in filteredCountries" class="w-full flex flex-col" @click="selectCountry(country)">
        <UButton color="white" variant="ghost"
          ...
          <span class="truncate font-normal text-sm sm:text-base">{{ index + 1 }}. {{ country.COUNTRY_NAME }}</span>
          <template #trailing>
