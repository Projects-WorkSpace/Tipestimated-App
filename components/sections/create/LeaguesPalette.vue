  ...
  <div class="w-full h-full bg-white flex flex-col gap-y-2 py-2">
    ...
    <ul class="w-full flex flex-col max-h-[34vh] overflow-y-auto">
      <li v-for="(country, index) in filteredLeagues" class="w-full flex flex-col">
        <UButton @click="selectLeague(country)" color="white" variant="ghost"
          class="border-b border-neutral-200 hover:bg-c-light transition-colors duration-200"
          :ui="{ rounded: 'rounded-none', padding: { sm: 'p-3' } }">
          <span class="truncate font-normal text-sm sm:text-base">{{ index + 1 }}. {{ country.LEAGUE_NAME }}</span>
          <template #trailing>
            <UIcon name="i-heroicons-chevron-right-20-solid" class="w-4 sm:w-5 h-4 sm:h-5 ms-auto" />
          </template>
        </UButton>
      </li>
    </ul>
...
const filteredLeagues = computed(
  () =>
    query.value === ""
      ...
      : props.leagues
        .filter((country) => {
          return country.LEAGUE_NAME.toLowerCase().includes(
