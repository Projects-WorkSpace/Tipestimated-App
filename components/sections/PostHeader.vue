...
const emits = defineEmits<{
  (e: "clickFollowToggle", payload: boolean, tipsterID: string): void;
}>();

const clickFollowToggle = (payload: boolean) => {
  emits("clickFollowToggle", payload, props.tipster.id);
};
</script>
<template>
  <div class="w-full card-header flex justify-between items-center">
    <div class="flex gap-x-4 items-center">
      <UiAvatar
        w-and-h="w-10 md:w-14 h-10 md:h-14"
        :img-url="tipster.imageUrl"
        :id-url="tipster.id"
        :user-name="tipster.penName"
        :name="tipster.user.userName"
        :tipster-followers="tipster.followerCount"
      />
      <span class="text-sm sm:text-base md:text-lg font-medium">{{
        tipster.penName
      }}</span>
    </div>
