...
import footballResults from '~/constants/footballResults';
import { IExactScore, IOverUnder } from '~/types/plays';
...
const possibleScores = computed(() => footballResults.sort((a, b) => a.id - b.id));

const selectMWBtn = (value: string) => {
  emits('updatePredictionScore', 'mw', { value: value })
}
const selectBothToScore = (value: string) => {
  emits('updatePredictionScore', 'gg', { value: value });
}


const inputExactHomeValue = (e: Event) => {
  let data = (e.target as HTMLInputElement).value
  emits('updatePredictionScore', 'exactScore', { home: data, away: props.exactScoreValue?.away || 0 })
}
const inputExactAwayValue = (e: Event) => {
  let data = (e.target as HTMLInputElement).value;
  emits('updatePredictionScore', 'exactScore', { home: props.exactScoreValue?.home || 0, away: data });
}
  ...
  <div class="w-full flex flex-col">
    <ul class="w-full flex flex-col gap-y-4">
      <li v-for="score in possibleScores" class="w-full flex flex-col gap-y-2.5" :key="score.id">
        ...
        <ul v-else-if="score.id === 2" class="w-full grid gap-3 grid-cols-2">
          <li v-for="value, index in score.values" class="w-full flex flex-col" :key="index">
            <button @click="selectBothToScore(value.value)"
              :class="bothToScoreValue === value.value ? 'bg-green-500 text-white hover:bg-green-600' : 'bg-c-seperator/60 text-neutral-600 hover:bg-c-seperator/90'"
              class="px-3.5 py-1.5  text-sm rounded-md ring-1 ring-c-seperator  transition-colors duration-200">
              {{ value.value }}
            </button>
          </li>
        ...
        <div v-else-if="score.id === 3" class="w-full flex flex-col gap-y-2">
          ...
          <div class="w-full grid grid-cols-2 items-center gap-x-3">
            <input :value="exactScoreValue?.home" @input="inputExactHomeValue" type="number" min="0" max="10"
            ...
            <input :value="exactScoreValue?.away" @input="inputExactAwayValue" type="number" min="0" max="10"
              class="w-full py-2 px-4 ring-1 ring-c-seperator rounded-md bg-white placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-neutral-400 transition-colors duration-200"
              placeholder="Type score here..." />
          </div>
        </div>
        <div v-else-if="score.id === 5" class="w-full flex flex-col gap-y-2">
          <div class="w-full grid grid-cols-2 items-center gap-x-3">
