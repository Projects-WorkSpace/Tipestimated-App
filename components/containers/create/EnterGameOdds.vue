...
<template>
  <div class="w-full">
    ...
    >
      <div
        class="w-full h-full flex flex-col justify-between py-6 px-4 gap-y-2 relative min-h-[45vh]"
      >
        <button
          @click="closeModal"
          class="absolute right-6 text-neutral-600 hover:text-neutral-800"
        >
          <Icon name="mdi:close" class="text-xl" />
        </button>
        <div class="w-full flex flex-col gap-y-5">
          <label for="odds" class="text-lg text-center font-medium"
            >Enter odds</label
          >
          <div class="w-full flex flex-col mt-4">
            <input
              :value="predictionOdds"
              @input="updatePredictedOdds"
              id="odds"
              type="number"
