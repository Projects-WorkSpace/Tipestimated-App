...
<template>
  <div class="w-full">
    <button @click="openModal" :class="leagueData === null
      ...
      <Transition mode="out-in">
        <div v-if="matchData" class="w-full flex items-center justify-center truncate gap-x-3">
          <div class="flex gap-x-1 items-center">
            <NuxtImg :src="matchData?.HOME_IMAGES[0] || ''" class="w-4 h-4" />
            <span class="text-sm font-normal tracking-wide text-ellipsis overflow-hidden">{{ matchData?.HOME_NAME
            }}</span>
          </div>
          ...
          <div class="flex gap-x-1 items-center truncate">
            ...
            <p class="text-sm font-normal tracking-wide text-ellipsis overflow-hidden">
              {{ matchData?.AWAY_NAME }}
            </p>
          </div>
        </div>
    ...
    <UModal v-model="isOpen" :overlay="false" :ui="{
      ...
      <SectionsCreateGamesList :league-data="leagueData" @select-event="selectEvent" />
    </UModal>
  </div>
</template>
