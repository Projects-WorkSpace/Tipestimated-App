...
const {
  mutate: postFollowData,
  onDone: onFollow,
  onError: onFollowError,
} = useMutation<IFollowTipster>(FollowTipster);
const {
  mutate: postUnFollowData,
  onDone: onUnFollow,
  onError: onUnFollowError,
} = useMutation<IUnFollowTipster>(UnFollowTipster);
      ...
      <div class="flex flex-col items-center gap-y-4 grow">
        <div class="w-full flex flex-col md:flex-row md:items-center justify-between gap-y-4 md:gap-y-2.5">
          <div class="flex md:flex-col items-baseline md:items-start gap-x-2">
            <span class="text-sm md:text-lg md:font-medium lowercase"><span class="font-normal text-base">@</span>{{
          ...
          <div class="flex items-center justify-between gap-x-1.5 md:gap-x-2.5">
            ...
            <div class="">
              <button
                class="bg-c-seperator/60 text-neutral-900 font-medium text-sm rounded-lg px-3 sm:px-4 md:px-6 py-1.5 md:py-2 hover:bg-c-seperator transition duration-200">
                Message
              </button>
            </div>
