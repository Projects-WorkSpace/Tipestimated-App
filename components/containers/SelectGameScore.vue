...
const updatePredictionScore = (key: string, value: any) => {
  if (key === 'mw') {
    ...
    emits('updatePredictionScore', { name: 'bothTeamToScore', value: value.value })
...
<template>
  <div class="w-full">
    ...
    <UModal v-model="isOpen" :overlay="false" :ui="{ width: 'w-[110%]', height: 'min-h-[60vh]', background: 'bg-white' }">
      <SectionsScoresList :match-data="matchData" :match-winner-score-value="matchWinnerScoreValue"
