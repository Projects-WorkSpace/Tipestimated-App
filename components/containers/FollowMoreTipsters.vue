...
const fetchTipsters = async () => {
  ...
  const { onResult, onError } = useQuery<ITipsters>(TipsterFollowers, { first: 20 });
  onResult((result) => {
    loading_search.value = false;
    tipsterList.value = result.data.allTipsters.edges?.map((edge) => edge.node) ?? []
  })
  onError((error) => {
    loading_search.value = false;
...
const searchTipsters = async () => {
  loading_search.value = true;
  const { onResult, onError } = useQuery<ITipsters>(TipsterFollowers, { penName: searchText.value });
  onResult((result) => {
    loading_search.value = false;
    searchedData.value = result.data.allTipsters.edges?.map((edge) => edge.node) ?? []
  })
  onError((error) => {
    loading_search.value = false;
    console.log("Fetch error: ", error)
...
const updateFollowStatusSuggestions = (payload: boolean, tipsterId: string) => {
  let tipsterIndex = tipsterList.value.findIndex((edge) => edge.id === tipsterId)
  if (tipsterIndex !== -1) {
...
const updateFollowStatusSearched = (payload: boolean, tipsterId: string) => {
  let tipsterIndex = searchedData.value.findIndex((edge) => edge.id === tipsterId)
  if (tipsterIndex !== -1) {
